...
class StudentTest < ActiveSupport::TestCase
  ...
  test "Should raise error saving student with a picture that is too large" do
    assert_raises ActiveRecord::RecordInvalid do
      ...
      Student.create!(first_name:"Bark",last_name:"Bork",school_email:"b@msudenver.edu",major:"BS",minor:"ET",image: {io: large_picture, filename: 'large_picture.png', content_type: 'image/png'})
    end
  end
  test "Should raise error saving student with the wrong file type" do
    assert_raises ActiveRecord::RecordInvalid do
      ...
      Student.create!(first_name:"Bark",last_name:"Bork",school_email:"b@msudenver.edu",major:"BS",minor:"ET",image: {io: wrong_file, filename: 'wrong_file.gif', content_type: 'image/gif'})
    end
  end
  test "Should set default profile picture when none is uploaded" do
    ...
    assert student.image.attached?, "Profile picture should be attached"

    assert_equal "default.png", student.image.filename.to_s, "The attached picture should be default.png"
    assert_equal "image/png", student.image.content_type, "The attached picture should be a png"
  end
end
